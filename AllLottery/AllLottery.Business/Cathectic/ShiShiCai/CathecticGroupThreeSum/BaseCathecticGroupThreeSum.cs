...
namespace AllLottery.Business.Cathectic.ShiShiCai.CathecticGroupThreeSum
{
    public abstract class BaseCathecticGroupThreeSum : BaseCathectic
    {
        ...
        public override long IsWinEach(string data, int[] openDatas)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                sum += openDatas[i + Number];
                if (!dic.ContainsKey(openDatas[i + Number]))
                {
                    dic.Add(openDatas[i + Number], true);
                }
            }
            if (sum == dataNum)
            {
